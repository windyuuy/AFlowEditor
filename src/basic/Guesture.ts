...
namespace kitten.guesture {
	export type TouchPoint = math.Vector3
	export const TouchPoint = math.Vector3
	...
	export class TouchPointQueue {
		...
		getMoveVector(): TouchPoint {
			...
			if (this.points.length > 1) {
				math.Vector.subDown(math.Vector.addUp(vec, this.points[0]), this.points[1])
			}
			return vec
		}
		/**
		...
		getMaxMoveVector(): TouchPoint {
			let vec = new TouchPoint(0)
			if (this.points.length > 1) {
				math.Vector.subDown(math.Vector.addUp(vec, this.points[0]), this.points[this.points.length - 1])
			}
			return vec
		}
	...
	export class DragGuestureInfo extends GuestureInfo {
		...
		getMoveVector(index: number = 0): TouchPoint {
			...
			if (points.length > 1) {
				math.Vector.subDown(math.Vector.addUp(vec, points[0]), points[1])
			}
			return vec
		}
		/**
		...
		getMaxMoveVector(index: number = 0): TouchPoint {
			...
			let vec = new TouchPoint(0)
			if (points.length > 1) {
				math.Vector.subDown(math.Vector.addUp(vec, points[0]), points[points.length - 1])
			}
			return vec
		}
	...
	export class ScaleGuestureInfo extends GuestureInfo {
		...
		getScaleInfo(): ScaleInfo {
			...
			let scaleN = math.Vector.distance(pt1_1, pt2_1) - math.Vector.distance(pt1_2, pt2_2)
			let dir = math.Vector.subDown(vec1.clone(), vec2)
			let center = math.Vector.addUp(pt1_2.clone(), pt2_2)
			let info: ScaleInfo = {
				center,
				dir,
				scaleN,
	...
	export class RotateGuestureInfo extends GuestureInfo {
		...
		getRotateDirection(): RotateInfo {
			...
			let distanceY = math.Vector.len(math.Vector.subDown(pt1.clone(), pt2))

			let vec1 = pq1.getMaxMoveVector()
			let vec2 = pq2.getMaxMoveVector()
			let distanceX = math.Vector.len(math.Vector.subDown(vec1.clone(), vec2))

			let th = Math.atan2(distanceX, distanceY)
			let center = math.Vector.addUp(pt1.clone(), pt2)
	...
	export class GuestureAnalyzer {
		...
		getSinglePointGuesture(): GuestureInfo | null {
			...
			if (pointQueues.length >= 1) {
				...
				if (math.Vector.len(pointQueue.getMaxMoveVector()) > config.dragDistanceMin) {
		...
		getTowPointGuesture(): GuestureInfo | null {
			...
			if (pointQueues.length == 2) {
				...
				let calcDistance = (index1: number, pt2: TouchPoint) => {
					...
					if (moveVec.x == 0) {
						return math.Vector.distance(pt, pt2)
					}
				...
				if (math.Vector.len(moveVec1) > config.dragDistanceMin || math.Vector.len(moveVec2) > config.dragDistanceMin) {
					// 计算点到直线距离
